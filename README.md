...
Целевые метрики (раздел 5 ТЗ) и достигнутые на контрольном наборе из 26 вопросов
(корпус — 185 PDF / 6305 чанков; OpenAI gpt-5-mini + text-embedding-3-small, RAG-fusion вкл.):

| Метрика | Цель | Факт |
|---|---|---|
| Точность ответов | ≥ 85% | **96–100%** |
| Доля ответов с валидной ссылкой на страницу | ≥ 95% | **100%** |
| Средняя задержка ответа | ≤ 20 c | **~11 c** |
| Галлюцинации на вопросах без данных | 0 | **0** |
| Утечки на off-topic вопросы | 0 | **0** |

> Задержка ~11 c — с включённым RAG-fusion (query-expansion, +1 LLM-вызов на запрос ради
> устойчивости поиска к формулировке). Без него (`QUERY_EXPANSION=0`) — ~6.5 c. Цель ≤ 20 c
> выполняется в обоих режимах. Точность варьируется в пределах 96–100% из-за `temperature=1`
> у gpt-5-mini (фиксированное значение, недетерминизм).
