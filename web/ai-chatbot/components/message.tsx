        ...
        >
          ...
          {message.parts?.map((part, index) => {
            ...
            if (type === "text") {
              ...
              const cleanedText = sourceMatch
                ? rawText.replace(sourceMatch[0], "").trim()
                : rawText;
              ...
              if (mode === "view") {
                return (
                  <div className="space-y-3" key={key}>
                    ...
                    {message.role === "assistant" && sourceUrl && (
                      <a
                        className={cn(
                          buttonVariants({ variant: "outline", size: "sm" }),
                          "w-fit rounded-full px-4 py-2 text-sm"
                        )}
                        href={sourceUrl}
                        rel="noreferrer"
                        target="_blank"
                      >
                        {sourceTitle ? `Источник: ${sourceTitle}` : "Источник"}
                      </a>
                    )}
                  </div>
                );
